...
func main() {
	...
	if err != nil {
		zap.L().Error("poc-fulfillment-service: failed to start", zap.String("error", err.Error()))
	}
}
...
func initializeService() (*grpc.Server, net.Listener, error) {
	...
	zap.L().Info("poc-fulfillment-service: starting server", zap.String("port", port))
	...
	if err != nil {
		...
		return nil, listener, fmt.Errorf("failed to initialize the FulfillmentService: %v", err)
	}

	// Initialize the gRPC server
	grpcServer := grpc.NewServer()
	pb.RegisterFulfillmentAPIServer(grpcServer, svc)
